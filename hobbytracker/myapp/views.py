...
"""This Function renders and saves our values for adding time to hobbies"""
def hobby_time_form(response):
    if response.method == "POST":
        ...
        if form.is_valid():
            ...
            times_raw = HobbyTime.objects.raw('SELECT id,SUM((julianday(endTime) - julianday(startTime)) * 24 * 60) totalmins,date(endTime) hobbydate FROM myapp_hobbytime WHERE hobby_id='+ str(hobbyid) +' AND startTime>=date(\'now\',\'-'+ str(days_back) +' day\') AND endTime<=date(\'now\',\''+ str(7 - days_back) +' day\') GROUP BY date(startTime)')
...
"""This passes our list of user hobbies and passes the form to create new hobbies"""
...
class HobbyChartView(TemplateView):
    """as view that contains all our sprite logic"""
